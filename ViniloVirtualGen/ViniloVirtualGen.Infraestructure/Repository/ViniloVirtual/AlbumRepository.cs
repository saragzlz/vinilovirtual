        ...
        public partial class AlbumRepository : BasicRepository, IAlbumRepository
        {
                ...
                public void AnyadirCompra(int p_Album_OID, string p_usuario_OID)
                {
                        ...
                        try
                        {
                                ...
                                albumEN = (AlbumEN)session.Load(typeof(AlbumNH), p_Album_OID);
                                ViniloVirtualGen.ApplicationCore.EN.ViniloVirtual.UsuarioEN usuarioENAux = null;
                                if (albumEN.Usuario_0 == null)
                                {
                                        albumEN.Usuario_0 = new System.Collections.Generic.List<ViniloVirtualGen.ApplicationCore.EN.ViniloVirtual.UsuarioEN>();
                                }


                                usuarioENAux = new ViniloVirtualGen.ApplicationCore.EN.ViniloVirtual.UsuarioEN();
                                usuarioENAux = (ViniloVirtualGen.ApplicationCore.EN.ViniloVirtual.UsuarioEN)session.Load(typeof(ViniloVirtualGen.Infraestructure.EN.ViniloVirtual.UsuarioNH), p_usuario_OID);
                                usuarioENAux.Album.Add(albumEN);

                                albumEN.Usuario_0.Add(usuarioENAux);
                ...
                public void EliminarListaAlbumesCompra(int p_Album_OID)
                {
                        ...
                        try
                        {
                                ...
                                foreach (UsuarioEN item in albumEN.Usuario_0)
                                {
                                        item.Album = item.Album.Where(x=> x.Id != p_Album_OID).ToList();
                                }
